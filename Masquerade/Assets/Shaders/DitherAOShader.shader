Shader "Custom/DitherAOShader"
{
    ...
    SubShader
    {
        ...
        float GetAODotPattern(float2 pixelPos, float scale, float aoIntensity) {
            // Remap AO intensity: darker AO = larger dots (inverted from lightIntensity logic)
            float dotSizeFactor = pow(1.0 - aoIntensity, _DotSizeGradient);
            float dynamicDotSize = lerp(_MinDotSize, _MaxDotSize, dotSizeFactor) * _DotSize;
    
            // Scale the position by the pattern scaling factor
            float2 scaledPos = pixelPos * scale * _DotTiling / (_ScreenParams.xy * _DotSpacing);
    
            // Apply row-based staggering
            float rowIndex = floor(scaledPos.y);
            float randOffset = _RowOffset;
            float xOffset = fmod(rowIndex, 2.0) * randOffset;
    
            // Apply the offset to create the staggered grid
            float2 staggeredPos = float2(scaledPos.x + xOffset, scaledPos.y);
    
            // Determine cell and position within cell
            float2 cell = floor(staggeredPos);
            float2 cellUV = frac(staggeredPos);
            ...
            // Use a different bayer pattern offset for each cell
            uint x = fmod(cell.x, 4);
            uint y = fmod(cell.y, 4);
            float bayerValue = BAYER_PATTERN[y * 4 + x];
        ...
        Pass
        {
            ...
            half4 frag(V2F input) : SV_Target
            {
                ...
                // Apply AO dithering effect
                // Only apply AO effect where AO is significant (below threshold)
                if (aoValue < _AOToHatchThreshold) {
                    float2 pixelPos = input.positionCS.xy;
        
                    // Get AO intensity for dot pattern (inverted - darker AO = larger dots)
                    float aoIntensityForDots = saturate(((_AOToHatchThreshold- aoValue) / _AOToHatchThreshold) * 1.5);
        
                    // Get dot pattern specific for AO
                    float aoDotPattern = GetAODotPattern(pixelPos, _DitherScale * 0.8, aoIntensityForDots);
        
                    // Apply AO dot pattern
                    if (aoDotPattern > 0) {
                        float aoDotIntensity = aoDotPattern * _HatchIntensity;
                        finalColor = lerp(finalColor, shadowColor, aoDotIntensity);
                    }
                    //finalColor = lerp(finalColor, darkColor, 1 - aoValue);
                }
