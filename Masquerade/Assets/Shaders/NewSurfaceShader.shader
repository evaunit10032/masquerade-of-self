Shader "Custom/HatchDitherShader2"
{
    Properties
    {
        ...
        _HatchTint ("Hatching Texture Tint", Color) = (1,1,1,1)
        ...
        _HatchTiling ("Hatching Tiling", Vector) = (1,1,0,0)
        ...
        _AOStrength ("AO Strength", Range(0, 100)) = 1.0
        _AOToHatchThreshold ("AO to Hatch Threshold", Range(0, 1)) = 0.5
        _HatchFadeDistance ("Hatch Fade Distance", Range(0.1, 100.0)) = 3.0
        _HatchIntensity ("Hatch Intensity", Range(0, 1)) = 0.7
    ...
    SubShader
    {
        ...
        CBUFFER_START(UnityPerMaterial)
            ...
            float _DotSpacing;
            float _AOStrength;
            float _AOToHatchThreshold;
            float _HatchFadeDistance;
        ...
        // Get AO value from a screen position
        float SampleAO(float2 uv) {
            // In URP, we need to access AO from the AmbientOcclusionFactor struct
            #ifdef _SCREEN_SPACE_OCCLUSION
                // We don't call SampleSceneAmbientOcclusion directly as it's not directly exposed in all URP versions
                // Instead we'll handle this in the fragment shader using GetScreenSpaceAmbientOcclusion
                return 1.0;
            #else
                return 1.0;
            #endif
    ...
    return 0.0; // No dots in fully lit areas
}
        ...
        float2 MapHatchingToAOArea(float2 uv, float3 normalWS, float aoValue) {
            // Determine the object-space up direction (could be different based on your setup)
            float3 objectUp = float3(0, 1, 0);
            float3 worldUp = normalize(objectUp);
    
            // Project the normal onto the up axis to determine the "height" on the object
            float heightFactor = dot(normalWS, worldUp);
    
            // Determine the base and tip positions based on AO intensity
            // This gives us the range of AO on this face
            float aoRange = 2    - aoValue; // Higher value = larger AO area
    
            // Calculate new texture coordinates
            // This remaps the vertical texture coordinate to stretch from the base to the tip of the AO area
            float2 remappedUV = uv; 
            remappedUV.y = lerp(0, 1, (remappedUV.y - (0.1 - aoRange/2)) / aoRange);
    
            // Clamp to ensure we only show the texture within the AO area
            remappedUV.y = saturate(remappedUV.y);
        ...
        Pass
        {
            ...
            V2F vert(AppData input)
            {
                ...
                output.positionNDC = output.positionCS * 0.5;
            ...
            half4 frag(V2F input) : SV_Target
            {
                ...
                // Apply AO strength
                aoValue = lerp(1.0, aoValue, _AOStrength);

                // Calculate hatching factor from AO 
                // Invert AO value so darker AO creates more hatching
                float hatchFactor = saturate((1.0 - aoValue - _AOToHatchThreshold) * 6.0);
                ...
                float4 hatchColor = SAMPLE_TEXTURE2D(_HatchTex, sampler_HatchTex, screenUV * _HatchTiling) * _HatchTint;
                ...
                // Apply hatching based on AO
                if (hatchFactor > 0) {
                 // Keep the original remapped UVs to maintain proper face fitting
                    float2 remappedUVs = MapHatchingToAOArea(screenUV * _HatchTiling, input.normalWS, aoValue);
    
                    // Sample the hatching texture with the remapped UVs
                    float4 hatchTexture = SAMPLE_TEXTURE2D(_HatchTex, sampler_HatchTex, remappedUVs) * _HatchTint;
    
                    // Create a hard threshold for where hatching can appear based on AO
                    float aoMask = step(_AOToHatchThreshold, 1.0 - aoValue);
    
                    // Create a hard threshold for the hatching lines themselves
                    // This removes all partial transparency in the hatching texture
                    float lineThreshold =1; // Adjust this value to control line thickness
                    float hardLine = step(lineThreshold, hatchTexture.a);
    
                    // Only apply hatching where both conditions are met:
                    // 1. Inside the AO-affected area
                    // 2. On a visible line
                    float combinedMask = aoMask * hardLine;
    
                    // Apply hatching with hard edges
                    if (combinedMask > 0) {
                        // Use full intensity to avoid any fuzziness
                        finalColor = lerp(finalColor, shadowColor, _HatchIntensity);
                    }
                }
                ...
                #ifdef _RECEIVE_SHADOWS
    // Apply shadows - lower values mean more shadow
    if(totalShadow < 0.5) // Adjust threshold as needed 
    {
        finalColor = shadowColor;
    }
#endif
        ...
        // DepthOnly pass - required for SSAO to work properly
        Pass
        {
            Name "DepthOnly"
        ...
        // DepthNormals pass - required for SSAO to work properly
        Pass
        {
            Name "DepthNormals"
